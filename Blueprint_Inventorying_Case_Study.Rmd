...
# Filter data for only top 8 manufacturers and get top 5 items for each
final_data <- top_data %>%
...
plot_list <- lapply(unique(final_data$Manufacturer_Trimmed), function(manufacturer) {
  total_items <- sum(final_data$Total_Quantity)
  ...
  ggplot(data, aes(x = reorder(item_display, Total_Quantity), y = Total_Quantity, fill = item_display)) +
    geom_col() +
    labs(title = paste0(manufacturer, " (n= " ,total_items, ")"), x = "Item", y = "Total Quantity") +
    theme_minimal() +
...
In conclusion, the case study for Blueprints has revealed significant inefficiencies in the current inventory management processes of the medical surplus recovery organization. The meticulous analysis of over 67,000 items and more than 2,000 data points highlighted the urgent need for standardized data entry and the consolidation of naming conventions to ensure accuracy and reliability in inventory tracking.
...
Next steps for our inventory process would be creating a more standardized way of categorizing our items. From the app design team, this data will help to quicken the process of sorting by creating a drop down menu that will change based off of the data collected here. 
...
<div align="center">
  <a href="https://www.linkedin.com/in/russell-day" style="flex: 0 0 auto;">LinkedIn</a>
  <a href="https://www.linkedin.com/in/russell-day" style="flex: 0 0 auto;">LinkedIn</a>
</div>
